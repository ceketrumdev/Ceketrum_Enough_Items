...
public class BrewingRecipeManager {
    ...
    private final Map<ResourceLocation, List<BrewingRecipe>> cache = new HashMap<>();
    ...
    public synchronized void ensureCacheBuilt() {
        ...
        if (server == null && client.player != null && client.player.level() != null) {
            try {
                Method getServerMethod = client.player.level().getClass().getMethod("getServer");
                server = (MinecraftServer) getServerMethod.invoke(client.player.level());
            } catch (Exception e) {}
        }
        if (server == null && client.level != null) {
            try {
                Method getServerMethod = client.level.getClass().getMethod("getServer");
                server = (MinecraftServer) getServerMethod.invoke(client.level);
            } catch (Exception e) {}
        }
        ...
        try {
            ...
            var allPotions = BuiltInRegistries.POTION.holders().toList();
            int totalRecipes = 0;
                    ...
                    for (Item ingredientItem : brewingIngredients) {
                        ...
                        try {
                            ...
                            if (Boolean.TRUE.equals(hasRecipe)) {
                                ...
                                if (!result.isEmpty()) {
                                    ResourceLocation resultId = FavoriteItemsManager.getUniqueItemId(result);
    ...
    public List<BrewingRecipe> getRecipesForOutput(ItemStack output) {
        ensureCacheBuilt();
        ResourceLocation outputId = FavoriteItemsManager.getUniqueItemId(output);
