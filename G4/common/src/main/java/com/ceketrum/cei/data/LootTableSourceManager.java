...
public class LootTableSourceManager {
    ...
    // On ne retient QUE des ResourceLocation, deja crees par le registre et
    // donc partages : l'index ne fait qu'y renvoyer. Les libelles lisibles
    // etaient auparavant stockes pour les 1710 items indexes alors que le
    // joueur n'en consulte qu'une poignee ; ils sont maintenant formates a la
    // demande et memorises dans les deux caches ci-dessous.
     ...
     * hierarchie -- de l'ordre du million de Field jetables sur 1818 tables.
     * Une classe donnee n'est desormais analysee qu'une seule fois.
     */
    ...
    public synchronized void ensureCacheBuilt() {
        // Module coupe : le scan des tables du pack n'a pas lieu du tout.
        ...
        try {
            // Les loot tables ne vivent PAS dans server.registryAccess() : elles
            // sont rechargeables, donc dans ReloadableServerRegistries. D'ou le
            // "Missing registry: minecraft:loot_table" que le diagnostic a montre
            // sur 1.21.1 comme sur 26.3. Holder.get() renvoie un RegistryAccess,
            // le reste du code est inchange.
            var registryManager = server.reloadableRegistries().get();
            ...
            for (var entry : lootTableRegistry.holders().toList()) {
                LootTable lootTable = entry.value();
            ...
            isCacheBuilt = true;
            com.ceketrum.cei.diag.CeiDiagnostics.heapDelta("Index des loot tables", cei$h0);
    ...
    private void scanObjectRecursively(Object obj, Set<Item> items, Set<Object> visited, int depth) {
        ...
        if (obj instanceof net.minecraft.tags.TagKey<?> tagKey) {
            try {
                if (tagKey.registry().location().getPath().equals("item")) {
                    ...
                    var optionalList = BuiltInRegistries.ITEM.getTag(itemTagKey);
        ...
        // n'entre plus dans les classes du paquet java.* : sans ce cas,
        // les valeurs facultatives des loot tables devenaient invisibles.
    ...
    private static synchronized java.lang.reflect.Field[] scannableFields(Class<?> clazz) {
        ...
        while (current != null && current != Object.class) {
            ...
            // haut (cas 6 a 8) : descendre dans leurs entrailles ne rapporte
            // rien et coute cher.
            ...
            if (client != null && client.player != null) {
                ...
                if (server != null) {
                    ...
                    if (lootTableRegistry != null) {
                        LootTable table = lootTableRegistry.get(id);
    ...
    public List<String> getSourcesForItem(Item item) {
        // Module coupe. L'onglet Butin disparait de lui-meme : updateMainTabs ne l'ajoute que sur une liste non vide.
