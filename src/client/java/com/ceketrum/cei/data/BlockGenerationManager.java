...
public class BlockGenerationManager {
    ...
    public synchronized void ensureCacheBuilt() {
        ...
        try {
            ...
            for (var biomeEntry : biomeRegistry.streamEntries().toList()) {
                var biome = biomeEntry.value();
                Identifier biomeId = biomeEntry.registryKey().getValue();
                ...
                for (int step : stepsOfInterest) {
                    try {
                        var features = genSettings.getFeatures();
    ...
    private void scanPlacedFeature(PlacedFeature feature, String biomeName, boolean isFr) {
        try {
            ...
            for (PlacementModifier modifier : feature.placementModifiers()) {
                if (modifier instanceof HeightRangePlacementModifier heightModifier) {
    ...
    private void scanFeatureConfigRecursively(Object config, Set<Block> blocks, Set<Object> visited, int depth) {
        ...
        if (config instanceof OreFeatureConfig.Target target) {
            blocks.add(target.state.getBlock());
            return;
        }
        
        // Extraction depuis BlockStateProvider
        if (config instanceof net.minecraft.world.gen.stateprovider.BlockStateProvider provider) {
            try {
    ...
    private String formatHeightModifier(HeightRangePlacementModifier modifier, boolean isFr) {
        try {
