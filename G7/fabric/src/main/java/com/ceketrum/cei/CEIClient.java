...
public class CEIClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		// 26.x : HudRenderCallback a disparu de Fabric API. Le HUD est desormais une
		// liste d'elements nommes ; on ajoute le notre en dernier pour qu'il passe
		// au-dessus du HUD vanilla.
				...
				for (var card : manager.getPinnedCards()) {
					if (card.isShowInHud()) {
						...
						if (pinnedScreen != null) {
							...
							pinnedScreen.extractRenderState(drawContext, -999, -999, 1.0f);
						}
					}
				}
			}
