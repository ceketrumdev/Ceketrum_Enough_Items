...
public class CeiConfigScreen extends Screen {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(this.font, shown, searchX + 7,
        ...
        context.text(this.font, titre, x1 - tw, y0 + (searchH - 8) / 2,
                           0xFFFFD700, false);
        ...
        for (Cat cat : Cat.values()) {
            ...
            context.text(this.font, cat.label(), catX + 10, cy + 6,
        ...
        for (int i = scroll; i < vis.size() && ry + rowH <= listY + listH; i++) {
            ...
            context.text(this.font, o.label(), listX + 10, ry + 5,
                               0xFFEEEEEE, false);
            String val = o.value();
            ...
            int couleur = o.kind == 0
                    ...
                    : 0xFFDDDDDD;
            context.text(this.font, val, listX + listW - 10 - vw, ry + 5,
                               couleur, false);
        ...
        if (vis.size() > perPage) {
            ...
            context.text(this.font, pos, listX + listW - 10 - pw,
        ...
        if (survolee != null) {
            context.text(this.font, survolee.tip(), listX,
                               listY + listH + 6, 0xFFAAAAAA, false);
        } else if (vis.isEmpty()) {
            context.text(this.font, CeiText.t("cei.config.empty"), listX,
    ...
    private void drawPanel(GuiGraphicsExtractor context, int x, int y, int w, int h, int bg) {
        context.fill(x, y, x + w, y + h, bg);
        context.outline(x, y, w, h, 0x33FFFFFF);
    }

    private void drawButton(GuiGraphicsExtractor context, int x, int y, String label,
        ...
        context.text(this.font, label, x + (buttonW - lw) / 2, y + 6,
                           accent ? 0xFFFFD700 : 0xFFEEEEEE, false);
    }

    private void drawTri(GuiGraphicsExtractor context, int x, int y, boolean haut) {
        int c = 0xFFAAAAAA;
        if (haut) {
