...
public class CeiModule {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY,
        ...
        if (pendingScreen != null) {
            ...
            com.ceketrum.cei.gui.util.CeiScreens.set(next);
    ...
    public boolean handleMouseClick(double mouseX, double mouseY, int button,
        ...
        if (panelRenderer.isSettingsButtonClicked((int) mouseX, (int) mouseY, ceiX, ceiY, ceiWidth)) {
            ...
            if (com.ceketrum.cei.gui.util.CeiScreens.current() != null) {
                ...
                pendingScreen = new CeiConfigScreen(com.ceketrum.cei.gui.util.CeiScreens.current());
            }
            return true;
        }
    ...
    private List<ItemStack> getFilteredItems() {
        ...
        if (filteredItemsCache != null
                ...
                && filterState.equals(lastFilterState)) {
        ...
        // Les filtres s'appliquent APRES la recherche : ils reduisent un
        // resultat, ils ne le remplacent pas.
        var filters = panelRenderer.getFilterPanel();
        filters.build(SHARED_ITEMS);
        if (filters.isActive()) {
            List<ItemStack> kept = new ArrayList<>();
            for (ItemStack stack : result) {
                if (filters.accepts(stack)) kept.add(stack);
            }
            result = kept;
        }
