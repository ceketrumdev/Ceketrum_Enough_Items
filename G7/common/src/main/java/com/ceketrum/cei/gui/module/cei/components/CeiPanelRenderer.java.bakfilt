...
public class CeiPanelRenderer {
     ...
     */
    public void render(GuiGraphicsExtractor context, int screenHeight, int screenWidth, net.minecraft.client.gui.Font textRenderer, 
        ...
        context.fill(ceiX + shadowOffset, ceiY + shadowOffset, 
                     ...
                     shadowAlpha << 24);
        context.fill(ceiX, ceiY, ceiX + ceiWidth, ceiY + ceiHeight, (bgAlpha << 24) | (config.getBackgroundColor() & 0xFFFFFFFF));
        context.outline(ceiX, ceiY, ceiWidth, ceiHeight, (borderAlpha << 24) | (config.getBorderColor() & 0xFFFFFFFF));
        ...
        context.text(textRenderer, Component.literal(title), 
        ...
        if (rotationAngle > 0.1f || rotationAngle < -0.1f) {
            ...
            // Rotation autour de l'axe Z (perpendiculaire à l'écran) en radians
            float rotationRad = (float) Math.toRadians(rotationAngle);
            context.pose().rotate(rotationRad);
            context.pose().translate(-centerX, -centerY);
        }
        
        context.text(textRenderer, Component.literal(settingsIcon), settingsButtonX, settingsButtonY, settingsColor, false);
        ...
        context.outline(favoritesButtonX, favoritesButtonY, favoritesButtonWidth, favoritesButtonHeight, 
        ...
        context.text(textRenderer, Component.literal(favoritesButtonText).withStyle(ChatFormatting.BOLD), 
