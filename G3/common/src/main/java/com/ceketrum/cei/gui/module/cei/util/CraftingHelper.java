...
public class CraftingHelper {
    ...
    private static void clearCraftingGrid(CraftingMenu handler, LocalPlayer player) {
        ...
        for (int slot = 1; slot <= 9; slot++) {
            ...
            if (!stack.isEmpty()) {
                ...
                client.gameMode.handleInventoryMouseClick(handler.containerId, slot, 0, ClickType.PICKUP, player);
                ...
                while (!cursorStack.isEmpty() && !placed) {
                    ...
                    for (int invSlot = 10; invSlot <= 36 && !cursorStack.isEmpty(); invSlot++) {
                        ...
                        if (!invStack.isEmpty() && canStack(invStack, cursorStack) && 
                            ...
                            client.gameMode.handleInventoryMouseClick(handler.containerId, invSlot, 0, ClickType.PICKUP, player);
                    ...
                    if (!foundCompatibleSlot || !cursorStack.isEmpty()) {
                        ...
                        if (!cursorStack.isEmpty()) {
                            client.gameMode.handleInventoryMouseClick(handler.containerId, 10, 0, ClickType.PICKUP, player);
                            placed = true;
                        }
                    }
                }
     ...
     */
    private static int calculateMaxCraftable(CraftingMenu handler, CraftingRecipe recipe,
                                             RegistryAccess registryManager,
                                             LocalPlayer player) {
        var ingredients = recipe.getIngredients();
        var inventory = player.getInventory();
        ItemStack resultStack = recipe.getResultItem(registryManager);
        ...
        java.util.List<java.util.Map.Entry<net.minecraft.world.item.crafting.Ingredient, Integer>> ingredientCounts = new java.util.ArrayList<>();
     ...
     */
    private static boolean ingredientsMatch(net.minecraft.world.item.crafting.Ingredient ing1, net.minecraft.world.item.crafting.Ingredient ing2) {
     ...
     */
    private static boolean placeShapedRecipe(CraftingMenu handler, ShapedRecipe recipe,
                                            RegistryAccess registryManager,
                                            LocalPlayer player, int quantity) {
        var ingredients = recipe.getIngredients();
                ...
                if (client != null && client.gameMode != null) {
                    try {
                        ...
                        if (!handler.getSlot(handlerSlot).getItem().isEmpty()) {
                            client.gameMode.handleInventoryMouseClick(handler.containerId, handlerSlot, 0, ClickType.PICKUP, player);
                        }
                        ...
                        for (int i = 0; i < 36 && !found; i++) {
                            ...
                            if (!stack.isEmpty() && ingredient.test(stack)) {
                                ...
                                if (quantity == 1) {
                                    ...
                                    client.gameMode.handleInventoryMouseClick(handler.containerId, inventoryHandlerSlot, 0, ClickType.PICKUP, player);
                                    // 2. Placer un seul item dans le slot de craft avec un clic droit
                                    client.gameMode.handleInventoryMouseClick(handler.containerId, handlerSlot, 1, ClickType.PICKUP, player);
                                    ...
                                    while (remainingToPlace > 0) {
                                        ...
                                        for (int j = 0; j < 36 && !foundStack && remainingToPlace > 0; j++) {
                                            ...
                                            if (!invStack.isEmpty() && ingredient.test(invStack)) {
                                                ...
                                                // Prendre le stack
                                                client.gameMode.handleInventoryMouseClick(handler.containerId, jHandlerSlot, 0, ClickType.PICKUP, player);
     ...
     */
    private static boolean placeShapelessRecipe(CraftingMenu handler, CraftingRecipe recipe,
                                               RegistryAccess registryManager,
                                               LocalPlayer player, int quantity) {
        var ingredients = recipe.getIngredients();
        var inventory = player.getInventory();
     ...
     */
    private static int findItemInInventory(net.minecraft.world.entity.player.Inventory inventory,
