...
public class CeiConfigScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer, shown, searchX + 7,
                           searchY + (searchH - 8) / 2, shownColor, false);
        if (queryFocus && (System.currentTimeMillis() / 500L) % 2L == 0L) {
            int cx = searchX + 7 + this.textRenderer.getWidth(query) + 1;
            context.fill(cx, searchY + 5, cx + 1, searchY + searchH - 5, 0xFFFFFFFF);
        }

        String titre = CeiText.t("cei.config.title");
        int tw = this.textRenderer.getWidth(titre);
        context.drawText(this.textRenderer, titre, x1 - tw, y0 + (searchH - 8) / 2,
                           0xFFFFD700, false);
        ...
        for (Cat cat : Cat.values()) {
            ...
            context.drawText(this.textRenderer, cat.label(), catX + 10, cy + 6,
        ...
        for (int i = scroll; i < vis.size() && ry + rowH <= listY + listH; i++) {
            ...
            context.drawText(this.textRenderer, o.label(), listX + 10, ry + 5,
                               0xFFEEEEEE, false);
            String val = o.value();
            ...
            int couleur = o.kind == 0
                    ...
                    : 0xFFDDDDDD;
            context.drawText(this.textRenderer, val, listX + listW - 10 - vw, ry + 5,
                               couleur, false);
        ...
        if (vis.size() > perPage) {
            ...
            context.drawText(this.textRenderer, pos, listX + listW - 10 - pw,
        ...
        if (survolee != null) {
            context.drawText(this.textRenderer, survolee.tip(), listX,
                               listY + listH + 6, 0xFFAAAAAA, false);
        } else if (vis.isEmpty()) {
            context.drawText(this.textRenderer, CeiText.t("cei.config.empty"), listX,
    ...
    private void drawPanel(DrawContext context, int x, int y, int w, int h, int bg) {
        context.fill(x, y, x + w, y + h, bg);
        context.drawBorder(x, y, w, h, 0x33FFFFFF);
    }
    ...
    private void drawButton(DrawContext context, int x, int y, String label,
        ...
        context.drawText(this.textRenderer, label, x + (buttonW - lw) / 2, y + 6,
                           accent ? 0xFFFFD700 : 0xFFEEEEEE, false);
    }

    private void drawTri(DrawContext context, int x, int y, boolean haut) {
        int c = 0xFFAAAAAA;
        if (haut) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseX >= listX && mouseX < listX + listW
            ...
            if (i >= 0 && i < vis.size()) {
                ...
                if (o.kind == 1) {
                    int tx = trackX(listX, listW, this.textRenderer.getWidth(o.value()));
