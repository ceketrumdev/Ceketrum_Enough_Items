...
public class HelpPopupRenderer {
    ...
    public void render(DrawContext context, int screenWidth, int screenHeight,
        ...
        int titleNeed = textRenderer.getWidth(title) + CLOSE_SIZE + CLOSE_MARGIN * 2;
        int need = Math.max(titleNeed,
                   Math.max(textRenderer.getWidth(helpText), textRenderer.getWidth(helpText2)))
                   + PADDING * 2;
        ...
        context.drawBorder(popupX, popupY, popupWidth, POPUP_HEIGHT, borderColor);
        ...
        int titleX = popupX + Math.max(PADDING, (titleZone - textRenderer.getWidth(title)) / 2);
        context.drawText(textRenderer, Text.literal(title).formatted(Formatting.BOLD),
                        titleX, popupY + 8, titleColor, false);

        // Texte d'aide
        int textColor = textAlpha << 24 | 0xFFFFFF;
        int textY = popupY + 25;
        int textX = popupX + (popupWidth - textRenderer.getWidth(helpText)) / 2;
        context.drawText(textRenderer, Text.literal(helpText), textX, textY, textColor, false);
        if (!helpText2.isEmpty()) {
            textX = popupX + (popupWidth - textRenderer.getWidth(helpText2)) / 2;
            context.drawText(textRenderer, Text.literal(helpText2), textX, textY + 12, textColor, false);
        }
        ...
        String closeSymbol = "×";
        int symbolWidth = textRenderer.getWidth(closeSymbol);
        ...
        int symbolY = closeButtonY + (closeButtonSize - textRenderer.fontHeight) / 2;
        context.drawText(textRenderer, Text.literal(closeSymbol), symbolX, symbolY, textColor, false);
    }
