...
public class CeiItemListRenderer {
     ...
     */
    public ItemStack render(DrawContext context, int mouseX, int mouseY,
        ...
        for (int i = startIndex; i < Math.min(allItems.size(), startIndex + maxVisibleItems); i++) {
            ...
            if (hoverScale != 1.0f) {
                ...
                context.getMatrices().scale(hoverScale, hoverScale, 1.0f);
                context.getMatrices().translate(-centerX, -centerY, 0);
            }

            // Calcul de l'offset pour centrer l'item (supposé 16x16)
            int offset = (GuiConstants.SLOT_SIZE - 16) / 2;
            context.drawItem(stack, x + offset, y + offset);
            ...
            if (favoriteToggleTimes.containsKey(itemId)) {
                ...
                context.getMatrices().push();
                context.getMatrices().translate(0, 0, 200); // Z-index élevé pour être au-dessus de l'item
                context.drawText(textRenderer, Text.literal(starIcon).formatted(Formatting.BOLD),
     ...
     */
    private void renderScrollBar(DrawContext context, int ceiHeight, int itemsListStartY, int maxScroll, int ceiWidth, int ceiX, int ceiY, float animationSlideOffset) {
        int animatedCeiX = (int) (ceiX + animationSlideOffset);
        barW = 4;
