...
public class TextRenderHelper {
     ...
     */
    public static String truncateText(String text, int maxWidth, TextRenderer textRenderer) {
        ...
        while (textRenderer.getWidth(truncated) > availableWidth && truncated.length() > 0) {
     ...
     */
    public static int drawWrappedText(DrawContext context, String text, int x, int y, int maxWidth, int color, float scale, int indent, TextRenderer textRenderer) {
        ...
        for (int p = 0; p < paragraphs.length; p++) {
            ...
            for (String word : words) {
                ...
                if (testWidth > maxWidth && currentLine.length() > 0) {
                    ...
                    context.drawText(textRenderer, Text.literal(currentLine.toString()), (int)(currentX * invScale), (int)(currentY * invScale), color, false);
                    context.getMatrices().pop();
                    
                    currentLine = new StringBuilder(word);
                    currentY += (int) (textRenderer.fontHeight * scale) + 2;
                    currentX = x + indent;
                } else {
