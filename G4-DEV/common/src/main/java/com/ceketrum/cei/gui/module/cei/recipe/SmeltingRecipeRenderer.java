...
public class SmeltingRecipeRenderer implements IRecipeRenderer {
    ...
    private int drawCookingArrowBlock(GuiGraphics context, int startX, int startY, 
                                     RecipeHolder<?> smeltingRecipeEntry, RegistryAccess dynamicRegistryManager) {
        ...
        if (possibleItems.length > 0) {
            ...
            context.renderItem(rawStack, blockStartX, startY);
        }
        ...
        if (smeltingRecipe instanceof SmeltingRecipe smelting) {
            output = smelting.getResultItem(dynamicRegistryManager);
        } else {
