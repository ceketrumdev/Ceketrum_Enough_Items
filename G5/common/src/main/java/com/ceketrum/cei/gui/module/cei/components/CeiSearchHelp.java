...
public class CeiSearchHelp {
    ...
    public void renderButton(GuiGraphics context, int x, int y,
        ...
        context.renderOutline(x, y, BUTTON_W, BAR_H, GuiConstants.BORDER_COLOR);
        String q = "?";
        context.drawString(textRenderer, Component.literal(q),
                x + (BUTTON_W - textRenderer.width(q)) / 2,
                y + (BAR_H - textRenderer.lineHeight) / 2 + 1,
    ...
    private void prepare(String searchText, java.util.List<String> mods,
                         java.util.List<String> tags, net.minecraft.client.gui.Font textRenderer,
    ...
    public void renderOverlay(GuiGraphics context, net.minecraft.client.gui.Font textRenderer,
                              double mouseX, double mouseY) {
        if (helpVisible) {
            ...
            List<String> lines = helpLines;
            int w = 0;
            for (String l : lines) w = Math.max(w, textRenderer.width(l));
            int h = lines.size() * (textRenderer.lineHeight + 1) + 6;
            ...
            context.renderOutline(x, y, w + 10, h, GuiConstants.BORDER_COLOR);
            int ly = y + 4;
            for (int i = 0; i < lines.size(); i++) {
                context.drawString(textRenderer, Component.literal(lines.get(i)), x + 5, ly,
                        i == 0 ? 0xFFFFFF55 : 0xFFCCCCCC, false);
                ly += textRenderer.lineHeight + 1;
            }
            return;
        }
        ...
        context.renderOutline(popX, popY, popW, h, GuiConstants.BORDER_COLOR);
        int ly = popY + 2;
        for (String id : shown) {
            ...
            context.drawString(textRenderer, Component.literal(id), popX + 4, ly + 1,
