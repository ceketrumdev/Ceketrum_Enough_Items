...
public final class CeiRecipeIndex {
    ...
    private static final Map<Item, List<net.minecraft.world.item.crafting.RecipeHolder<?>>> BY_OUTPUT = new HashMap<>();
    private static final Map<Item, List<net.minecraft.world.item.crafting.RecipeHolder<?>>> BY_INPUT = new HashMap<>();
    ...
    private static List<net.minecraft.world.item.crafting.RecipeHolder<?>> pending = null;
     ...
     */
    public static synchronized void ensureBuilt(Minecraft client) {
     ...
     */
    public static synchronized boolean buildStep(Minecraft client, long budgetNanos) {
        if (client == null || client.level == null) return false;

        Object owner = client.level;
        ...
        if (pending == null || pendingFrom != owner) {
            ...
            BY_OUTPUT.clear();
            BY_INPUT.clear();
            List<net.minecraft.world.item.crafting.RecipeHolder<?>> all = new ArrayList<>();
            all.addAll(com.ceketrum.cei.util.ClientRecipeHelper.getRecipes(client));
            pending = all;
            pendingFrom = owner;
            cursor = 0;
        }
        ...
        while (cursor < pending.size()) {
            indexOne(pending.get(cursor++), contextMap);
    ...
    public static synchronized List<net.minecraft.world.item.crafting.RecipeHolder<?>> producedBy(Item item) {
        List<net.minecraft.world.item.crafting.RecipeHolder<?>> list = BY_OUTPUT.get(item);
        return list == null ? Collections.emptyList() : list;
    }

    /** Recettes consommant cet item. Liste vide si aucune -- jamais null. */
    public static synchronized List<net.minecraft.world.item.crafting.RecipeHolder<?>> usedIn(Item item) {
        List<net.minecraft.world.item.crafting.RecipeHolder<?>> list = BY_INPUT.get(item);
        return list == null ? Collections.emptyList() : list;
    }
    ...
    private static void register(Set<Item> produced, Set<Item> consumed, net.minecraft.world.item.crafting.RecipeHolder<?> entry) {
    ...
    private static void indexOne(net.minecraft.world.item.crafting.RecipeHolder<?> entry, net.minecraft.util.context.ContextMap contextMap) {
        net.minecraft.world.item.crafting.Recipe<?> recipe = entry.value();
        ...
        try {
            for (var display : recipe.display()) {
                for (var inputSlot : com.ceketrum.cei.gui.screen.CeiItemInfoScreen
                        .getRecipeIngredients(display)) {
                    if (inputSlot == null) continue;
                    for (ItemStack stack : inputSlot.resolveForStacks(contextMap)) {
                        if (stack != null && !stack.isEmpty()) consumed.add(stack.getItem());
                    }
                }
            }
        } catch (Exception | LinkageError e) {
