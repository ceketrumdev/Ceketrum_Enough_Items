...
public class CeiItemInfoScreen extends Screen {
    ...
    private void drawHeaderButtons(DrawContext context, int mouseX, int mouseY) {
        ...
        context.drawBorder(calcX, calcY, 12, 12, calcOpen ? 0xFFFFD700 : 0x44FFFFFF);
    ...
    private void drawRecipeContent(DrawContext context, int mouseX, int mouseY, int contentX, int contentY, int contentWidth, int contentHeight) {
        ...
        switch (activeCategory.type) {
            case CRAFTING: {
                ...
                try {
                    ItemStack outStack = recipe.getOutput(rm);
    ...
    public static String itemTooltipText(ItemStack stack, int maxLines, boolean withInspector) {
        ...
        try {
            var lines = getTooltipFromItem(MinecraftClient.getInstance(), stack);
     ...
     */
    private void drawCalculator(DrawContext context, int mouseX, int mouseY,
        ...
        int qtyW = this.textRenderer.getWidth(qty);
        context.drawText(this.textRenderer, qty,
        ...
        if (result.noRecipe) {
            ...
            context.drawText(this.textRenderer, CeiText.t("cei.info.no_recipes"),
        ...
        if (calcHasArrows) {
            ...
            String pos = (calcScroll + 1) + "-" + Math.min(total, calcScroll + visible)
                    + "/" + total;
            int pw = this.textRenderer.getWidth(pos);
            int px = calcUpX - 4 - pw;
            // Le compteur cede la place au titre plutot que de l'ecraser.
            if (px > x + 60) {
                context.drawText(this.textRenderer, pos, px, rowY, 0xFF888888, false);
                titleRight = px - 6;
            }
        }
        ...
        if (result.truncated) {
            context.drawText(this.textRenderer, "...", x, bottom - 8, 0xFF888888, false);
        }
    }

    /** Le nom de l'objet vise, tronque a la place que lui laisse l'en-tete. */
    private void drawCalcTitle(DrawContext context, int x, int rowY, int right) {
        String title = TextRenderHelper.truncateText(
                targetStack.getName().getString(),
                Math.max(12, right - (x + 20)), this.textRenderer);
        context.drawText(this.textRenderer, title, x + 20, rowY, 0xFFFFD700, false);
    }
     ...
     * trait.
     */
    private void drawTreeRows(DrawContext context,
                              java.util.List<com.ceketrum.cei.gui.module.cei.util.CeiCraftTree.Row> rows,
        ...
        for (int i = calcScroll; i < end; i++) {
            ...
            int amountW = this.textRenderer.getWidth(amount);
            int amountRight = x + w - 14;
            int nameX = ix + 20;
            String name = TextRenderHelper.truncateText(
                    node.stack.getName().getString(),
                    Math.max(12, amountRight - amountW - 4 - nameX), this.textRenderer);
            ...
            context.drawText(this.textRenderer, name, nameX, rowY, nameColor, false);
            context.drawText(this.textRenderer, amount, amountRight - amountW, rowY,
    ...
    private void drawMiniButton(DrawContext context, int x, int y, String label,
                                int mouseX, int mouseY) {
        boolean hover = mouseX >= x && mouseX < x + 11 && mouseY >= y && mouseY < y + 11;
        context.fill(x, y, x + 11, y + 11, hover ? 0x66FFFFFF : 0x22FFFFFF);
        context.drawBorder(x, y, 11, 11, 0x44FFFFFF);
        int lw = this.textRenderer.getWidth(label);
        context.drawText(this.textRenderer, label, x + (11 - lw) / 2, y + 2, 0xFFFFFFFF, false);
    }
     ...
     * pour un bouton mort.
     */
    private void drawArrowButton(DrawContext context, int x, int y, boolean up,
                                 int mouseX, int mouseY) {
        boolean hover = mouseX >= x && mouseX < x + 11 && mouseY >= y && mouseY < y + 11;
        context.fill(x, y, x + 11, y + 11, hover ? 0x66FFFFFF : 0x22FFFFFF);
        context.drawBorder(x, y, 11, 11, 0x44FFFFFF);
    ...
    private void drawChevron(DrawContext context, int x, int y, boolean open, boolean hover) {
     ...
     */
    private void drawItemDescription(DrawContext context, int x, int y, int w, int h) {
        ...
        int end = TextRenderHelper.drawWrappedText(context, text, x, y - descScroll, w,
                0xDDFFFFFF, 0.75f, 0, this.textRenderer);
        context.disableScissor();
    ...
    public static List<ItemStack> extractCustomOutputs(Recipe<?> recipe, net.minecraft.registry.DynamicRegistryManager rm) {
        List<ItemStack> list = new ArrayList<>();

        // 1. Try standard getResult()
        try {
            ItemStack standardResult = recipe.getOutput(rm);
    ...
    private String getMachineLabel(Recipe<?> recipe) {
        ...
        try {
            typeId = Registries.RECIPE_TYPE.getId(recipe.getType());
        } catch (Exception e) {
