...
public final class CeiRecipeIndex {
    ...
    public static synchronized boolean buildStep(MinecraftClient client, long budgetNanos) {
        ...
        if (pending == null || pendingFrom != owner) {
            ...
            for (net.minecraft.recipe.Recipe<?> r : client.world.getRecipeManager().values()) {
                net.minecraft.recipe.RecipeEntry<?> wrapped =
                        new net.minecraft.recipe.RecipeEntry<>(r.getId(), r);
                all.add(wrapped);
            }
            pending = all;
            pendingFrom = owner;
