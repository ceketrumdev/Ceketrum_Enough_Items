...
public final class CeiRecipeAdapter {
    ...
    private static List<ItemStack> salvageOutputs(Recipe<?> recipe, DynamicRegistryManager registries) {
        for (java.lang.reflect.Field f : fieldsOf(recipe.getClass())) {
            if (!OUTPUT_NAMES.contains(f.getName().toLowerCase(java.util.Locale.ROOT))) continue;
            Object value;
            try { value = f.get(recipe); } catch (Exception e) { continue; }
    ...
    private static CeiRecipeView build(Recipe<?> recipe, DynamicRegistryManager registries) {
        try {
    ...
    private static List<ItemStack> outputsOf(Recipe<?> recipe, DynamicRegistryManager registries) {
        try {
            ItemStack out = recipe.getOutput(registries);
    ...
    private static Text titleOf(Recipe<?> recipe) {
        ...
        if (typeId == null) return Text.literal(CeiText.t("cei.recipe.generic"));
        return Text.literal(CeiRecipeStation.labelFor(typeId));
    }
}
