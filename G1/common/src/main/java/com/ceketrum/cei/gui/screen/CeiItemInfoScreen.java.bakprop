...
public class CeiItemInfoScreen extends Screen {
    ...
    private void scanRecipes() {
        ...
        for (var recipe : recipeManager.values()) {
            net.minecraft.recipe.RecipeEntry<?> entry = new net.minecraft.recipe.RecipeEntry<>(recipe.getId(), recipe);
            
            // 1. Recipes: Produces this item
            try {
                boolean producesItem = false;
                ItemStack standardResult = recipe.getOutput(registryManager);
