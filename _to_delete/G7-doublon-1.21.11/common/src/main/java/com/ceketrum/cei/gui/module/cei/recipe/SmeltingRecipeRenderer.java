...
public class SmeltingRecipeRenderer implements IRecipeRenderer {
    ...
    public int render(GuiGraphics context, int startX, int startY,
        ...
        String title = Component.translatable("recipe.cei.cooking.header").getString();
        int titleWidth = textRenderer.width(title);
        context.drawString(textRenderer, Component.translatable("recipe.cei.cooking.header"), 
                        startX + (GuiConstants.POPUP_WIDTH - titleWidth) / 2, startY, 0xFFFFFFFF, false);
        
        int currentY = startY + 12;
        currentY = drawCookingArrowBlock(context, startX, currentY, recipeEntry);
     ...
     */
    private int drawCookingArrowBlock(GuiGraphics context, int startX, int startY, 
        ...
        if (client.level != null) {
            ...
            var displays = smeltingRecipe.display();
            if (!displays.isEmpty()) {
                ...
                if (display instanceof FurnaceRecipeDisplay furnace) {
                    inputStack = furnace.ingredient().resolveForFirstStack(contextMap);
                }
            }
        }
        ...
        if (inputStack != null && !inputStack.isEmpty()) {
            context.renderItem(inputStack, blockStartX, startY);
        }
        ...
        ItemStack output = outputStack != null ? outputStack : ItemStack.EMPTY;
        context.renderItem(output, arrowStartX + arrowLength + arrowHeadSize + 6, startY); // Espacement de 6 pixels après la flèche
